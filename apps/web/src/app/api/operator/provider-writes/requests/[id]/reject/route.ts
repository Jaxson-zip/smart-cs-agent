...
export async function POST(request: Request, context: RouteContext) {
  ...
  const body = await readSafeRejectionBody(request);
  if (!body.ok) return body.response;

  const { id } = await context.params;
  const response = await proxyOperatorApi(
    request,
    `/v2/provider-writes/requests/${encodeURIComponent(id)}/reject`,
    {
      method: "POST",
  ...
  try {
    return NextResponse.json(
      toProviderWriteResponse(responseBody, ["rejected", "blocked", "failed"]),
    );
  } catch {
    return invalidRequestResponse();
  }
...
async function readSafeRejectionBody(request: Request) {
  let raw: unknown;
  try {
    ...
    return {
      ...
      response: NextResponse.json(
        { error: "Provider write rejection payload is invalid" },
        { status: 400 },
      ),
    };
  }

  const parsed = ProviderWriteRejectionRequestSchema.safeParse(raw);
  if (!parsed.success) {
    return {
      ok: false as const,
      response: NextResponse.json(
