...
- Production real-channel webhook intake fails closed at API startup when `NODE_ENV=production` and `REAL_CHANNEL_WEBHOOKS_ENABLED=true` are set without configured secrets, a positive rate limit, an explicit freshness window, and channel queue thresholds.
