...
const failures = [];
const COMMAND_NAME = "verify:provider-write-graduated-rollout-approval";
...
function main() {
  ...
  if (failures.length > 0) {
    console.error("Provider write graduated rollout approval verification failed:");
  ...
  console.log("Provider write graduated rollout approval verification passed.");
  if (approval) {
...
function readStaticContent() {
  const files = {
    ...
    closeoutDocs: "docs/deploy/provider-write-controlled-expansion-closeout-review.md",
...
function verifyStaticArtifacts(content) {
  mustContainAll("package scripts", content.packageJson, [
    ...
    "\"verify:provider-write-graduated-rollout-approval:safe\": \"node scripts/verify-provider-write-graduated-rollout-approval.mjs --from-env --require-pass\"",
    "scripts/verify-provider-write-graduated-rollout-approval.mjs",
  ]);

  mustContainAll("provider write graduated rollout approval docs", content.approvalDocs, [
  ...
  mustContainAll("closeout review docs reference graduated approval", content.closeoutDocs, [
    "PR77 Provider Write Graduated Rollout Approval Gate",
  ...
  mustContainAll("production launch verifier wiring", content.productionLaunchVerifier, [
    ...
    "verify-provider-write-graduated-rollout-approval.mjs",
    "verify:provider-write-graduated-rollout-approval",
    "provider write graduated rollout approval",
    "SMARTCS_PROVIDER_WRITE_GRADUATED_ROLLOUT_APPROVAL_FILE",
    "smart-cs-agent.provider-write-graduated-rollout-approval.v1",
  ...
  mustContainAll("task plan graduated rollout approval", content.taskPlan, [
    "PR77 - Provider Write Graduated Rollout Approval Gate",
    "verify:provider-write-graduated-rollout-approval",
  ]);

  mustContainAll("progress graduated rollout approval", content.progress, [
    "Started PR77 provider write graduated rollout approval gate",
  ]);

  mustContainAll("gitignore graduated rollout approval artifacts", content.gitignore, [
    "provider-write-graduated-rollout-approval-artifacts/",
  ]);
}
...
function validateApproval(value, options) {
  ...
  if (value.schemaVersion !== "smart-cs-agent.provider-write-graduated-rollout-approval.v1") {
    failures.push("schemaVersion must be smart-cs-agent.provider-write-graduated-rollout-approval.v1");
  }
  ...
  validatePrerequisiteEvidence(value.prerequisiteEvidence, options);
...
function validateTarget(target) {
  ...
  if (target.fromRolloutTrack !== "controlled_multi_merchant") {
    failures.push("target.fromRolloutTrack must be controlled_multi_merchant");
  }
  if (target.toRolloutTrack !== "graduated_multi_merchant") {
    failures.push("target.toRolloutTrack must be graduated_multi_merchant");
  }
  if (
...
function validateExpansionScope(scope) {
  ...
  validateFingerprintArray("expansionScope.merchantFingerprints", scope.merchantFingerprints, 3, 25);
  ...
  validateIntegerInRange("expansionScope.maxMerchants", scope.maxMerchants, 3, 25);
  if (
  ...
  validateIntegerInRange("expansionScope.maxDailyProviderWrites", scope.maxDailyProviderWrites, 1, 250);
  ...
  if (scope.automaticNextWaveEnabled !== false) {
    failures.push("expansionScope.automaticNextWaveEnabled must be false");
  }
}
...
function validateApprovalSection(approval, options) {
  ...
  if (!hasValue(approval.secondReviewerFingerprint)) {
    failures.push("approval.secondReviewerFingerprint must be present");
  }
...
function validatePrerequisiteEvidence(value, options) {
  if (!isRecord(value)) {
    failures.push("prerequisiteEvidence must be an object");
    return;
  }
  validateAllowedKeys(value, PREREQUISITE_EVIDENCE_KEYS, "prerequisiteEvidence contains unsupported field");
  validateSha256(
    "prerequisiteEvidence.providerWriteControlledExpansionCloseoutReviewSha256",
    value.providerWriteControlledExpansionCloseoutReviewSha256,
  );
  for (const key of PREREQUISITE_EVIDENCE_KEYS) {
...
function validateApprovalCloseoutBinding(approval, closeoutReview) {
  if (
    ...
    failures.push("prerequisiteEvidence.providerWriteControlledExpansionCloseoutReviewSha256 must match the closeout review file sha256");
  }
...
function applySafeEnvDefaults(args, env) {
  ...
  return {
    ...
    approval:
      args.approval ??
      env.SMARTCS_PROVIDER_WRITE_GRADUATED_ROLLOUT_APPROVAL_FILE,
    closeoutReview:
      args.closeoutReview ??
      env.SMARTCS_PROVIDER_WRITE_GRADUATED_ROLLOUT_APPROVAL_CLOSEOUT_REVIEW_FILE,
    runLedger:
      args.runLedger ??
      env.SMARTCS_PROVIDER_WRITE_GRADUATED_ROLLOUT_APPROVAL_RUN_LEDGER_FILE,
    requirePass:
      args.requirePass ||
      env.SMARTCS_PROVIDER_WRITE_GRADUATED_ROLLOUT_APPROVAL_REQUIRE_PASS === "true",
  };
}
