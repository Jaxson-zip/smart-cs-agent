...
const failures = [];
const COMMAND_NAME = "verify:provider-write-controlled-expansion-closeout-review";
...
const EVIDENCE_KEYS = new Set([
  "controlledExpansionRunLedgerVerifierPassed",
...
function main() {
  ...
  if (failures.length > 0) {
    console.error("Provider write controlled expansion closeout review verification failed:");
  ...
  console.log("Provider write controlled expansion closeout review verification passed.");
  if (review) {
...
function verifyStaticArtifacts(content) {
  mustContainAll("package scripts", content.packageJson, [
    ...
    "\"verify:provider-write-controlled-expansion-closeout-review:safe\": \"node scripts/verify-provider-write-controlled-expansion-closeout-review.mjs --from-env --require-pass\"",
    "scripts/verify-provider-write-controlled-expansion-closeout-review.mjs",
  ]);

  mustContainAll("provider write controlled expansion closeout review docs", content.closeoutDocs, [
  ...
  mustContainAll("production launch verifier wiring", content.productionLaunchVerifier, [
    ...
    "verify-provider-write-controlled-expansion-closeout-review.mjs",
    "verify:provider-write-controlled-expansion-closeout-review",
    "provider write controlled expansion closeout review",
    "SMARTCS_PROVIDER_WRITE_CONTROLLED_EXPANSION_CLOSEOUT_REVIEW_FILE",
    "smart-cs-agent.provider-write-controlled-expansion-closeout-review.v1",
...
function validateReview(value, options) {
  if (!isRecord(value)) {
    failures.push("provider write controlled expansion closeout review root must be an object");
    return;
  }
  validateAllowedKeys(
    value,
  ...
  if (value.schemaVersion !== "smart-cs-agent.provider-write-controlled-expansion-closeout-review.v1") {
    failures.push("schemaVersion must be smart-cs-agent.provider-write-controlled-expansion-closeout-review.v1");
  }
...
function validateRunSummary(summary, options) {
  ...
  if (summary.customerRejectedCompensationCount > 0 && summary.rejectedCompensationReviewed !== true) {
    failures.push("runSummary.rejectedCompensationReviewed must be true");
  }
}
...
function validateCloseout(closeout, options) {
  ...
  if (!DECISIONS.has(closeout.decision)) {
    failures.push("closeout.decision must be approved_for_next_expansion_review or rejected_needs_investigation");
  }
  if ((options.requirePass || hasValue(options.review)) && closeout.decision !== "approved_for_next_expansion_review") {
    failures.push("closeout.decision must be approved_for_next_expansion_review");
  }
  for (const key of [
    "customerImpactReviewed",
    "providerMutationReviewCompleted",
...
function validateRunLedgerSource(value) {
  ...
  if (value.evidence?.providerWriteControlledExpansionPreflightVerifierPassed !== true) {
    failures.push("run ledger.evidence.providerWriteControlledExpansionPreflightVerifierPassed must be true");
  }
  if (value.evidence?.postExpansionReviewCompleted !== true) {
    failures.push("run ledger.evidence.postExpansionReviewCompleted must be true");
  }
...
function validateReviewRunLedgerBinding(review, runLedger) {
  if (review?.artifactBindings?.providerWriteControlledExpansionRunLedgerSha256 !== runLedger.sha256) {
    failures.push("artifactBindings.providerWriteControlledExpansionRunLedgerSha256 must match the run ledger file sha256");
  }
...
function applySafeEnvDefaults(args, env) {
  ...
  return {
    ...
    review:
      args.review ??
      env.SMARTCS_PROVIDER_WRITE_CONTROLLED_EXPANSION_CLOSEOUT_REVIEW_FILE,
    runLedger:
      args.runLedger ??
      env.SMARTCS_PROVIDER_WRITE_CONTROLLED_EXPANSION_CLOSEOUT_REVIEW_RUN_LEDGER_FILE,
    requirePass:
      args.requirePass ||
      env.SMARTCS_PROVIDER_WRITE_CONTROLLED_EXPANSION_CLOSEOUT_REVIEW_REQUIRE_PASS === "true",
  };
}
